...
**Note:** The CLI tool is still named `openclaw` (upstream hasn't renamed yet), so CLI commands and internal config paths still use that name.
...
Note: The CLI is still named `openclaw` until upstream renames it:
```typescript
