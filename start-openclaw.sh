...
if (moonshotApiKey) {
    ...
    console.log('Configuring Moonshot/Kimi provider with API key');
    ...
    // Set Kimi as the primary model
    config.agents.defaults.model.primary = 'moonshot/kimi-k2.5';
    console.log('Moonshot/Kimi K2.5 configured as primary model');
} else if (isOpenAI) {
...
// Allow overriding the primary model via environment variable
if (process.env.OPENCLAW_PRIMARY_MODEL) {
    console.log('Overriding primary model to:', process.env.OPENCLAW_PRIMARY_MODEL);
